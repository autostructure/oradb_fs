  ...
  if $sid_in_running_ps == 'B' or $sid_in_running_ps == 'C' or $sid_in_running_ps == 'P' or $sid_in_running_ps == 'T' {
   ...
   if $sid_in_db_list == 'B' or $sid_in_db_list == 'C' {
    ...
    oradb_fs::sig_file{ "SI DB Signature file: ${db_name}" :
     ...
     sig_file_name    => "db${major_ver}_create_${db_name}_${short_home_path}",
    } ->
...
/*
 ->
    oradb_fs::db_instances{ "DB Instance RN:${db_name}" :
     db_name       => $db_name,
     working_dir   => "/opt/oracle/sw/working_dir/${home}",
     oracle_home   => $home_path,
     db_instances  => $facts["oradb_fs::db_instances_${home}"],
    } ->
    oradb_fs::sig_file{ "DB instances signature file: ${db_name}" :
     product          => 'Package db_instances for DB',
     sig_version      => '1.0',
     type             => 'base install',
     sig_desc         => 'db_instances package',
     global_name      => $db_name,
     sig_file_name    => "ora_db${major_ver}_dbinstances_v01_${db_name}",
    }
*/
   }
  }
