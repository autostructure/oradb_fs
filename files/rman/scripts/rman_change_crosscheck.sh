#!/usr/bin/env ksh
#
#  @(#)fs615/db/ora/rman/linux/rh/rman_change_crosscheck.sh, ora, build6_1, build6_1a,1.3:10/13/11:15:50:55
#  VERSION:  1.5
#  DATE:  04/26/13
#
...
function usage_exit
{
   echo "Usage:  rman_change_crosscheck.sh {-o[ORACLE_SID]} -bN"
   echo "        -o, Values for ORACLE_SID i.e. [a|ddb|rdb|tdb|admin]"
   echo "        -bN where N is the backupset number"
   echo '           -b "225, 230, 240"   will check three backupsets.'
   exit 1
}
...
program="rman_change_crosscheck.sh"
...

{
   ...
   cat > $RAO1 <<EOF2
     ...
     echo "
       ...
       change backupset $B_KEY crosscheck;
