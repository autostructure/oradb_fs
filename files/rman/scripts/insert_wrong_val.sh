#!/usr/bin/env ksh
#
#  @(#)fs615/db/ora/rman/linux/rh/insert_wrong_val.sh, ora, build6_1, build6_1a,1.1:9/5/11:15:39:12
#  VERSION:  1.1
#  DATE:  9/5/11:15:39:12
#
...
export RAO1=$TMP/insert_wrong_val.1.sh
export LOG1=$LOG_DIR/insert_wrong_val.1.$$.log
...
cat > $RAO1 <<EOF2
   ...
   sqlplus /nolog << EOF
      connect / as sysdba;
      insert into netbackup.tsm_test values (2);
      select * from netbackup.tsm_test;
      commit;
EOF

   sleep 2
   date "+%Y-%m-%d:%H:%M:%S" | tee /tmp/time_stamp_wrong_val.txt
}
